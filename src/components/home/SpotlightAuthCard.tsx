...
export const SpotlightAuthCard: React.FC<SpotlightAuthCardProps> = ({
  initialUser = DEFAULT_MOCK_USER,
}) => {
  const [user, setUser] = useState<SpotlightUser>(initialUser);
  ...
  return (
    <div className="bg-white rounded-lg p-4 sm:p-5 border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between h-full group/spotlight relative z-0">
      {/* Dynamic Status Pill with Toggle for Testing/Live Demo */}
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] font-mono uppercase tracking-wider font-bold text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-200/60">
          স্পটলাইট কার্ড (কাজ ১৪)
        </span>
        <button
          onClick={toggleLoginState}
          title="ক্লিক করে লগইন/লগআউট অবস্থা টেস্ট করুন"
          className="text-[10px] text-gray-500 hover:text-gray-900 underline cursor-pointer"
        >
          {user.isLoggedIn ? '🔄 লগআউট মোড' : '🔄 লগইন মোড'}
        </button>
      </div>
      ...
      {!user.isLoggedIn ? (
        ...
        <div className="flex-1 flex flex-col justify-between py-1">
          <div>
            ...
            {user.recentCategory && (
              <Link
                href={user.recentCategory.targetUrl}
                prefetch={true}
